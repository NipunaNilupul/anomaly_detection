...
# FULL LIST of all 15 MVTec AD categories [cite: 800]
MVTEC_CATEGORIES = [
...
def compute_pro(anomaly_maps, gt_masks, fpr_thresh=0.3): [cite: 59]
    ...
    labels = labels[sorted_idx] [cite: 801]
    ...
    return tpr[valid[-1]] if len(valid) > 0 else tpr[0] [cite: 60]

def compute_aupro(anomaly_maps, gt_masks): [cite: 60]
    ...
    return auc(fpr_vals, pro_vals) / 0.3 [cite: 61]

def compute_image_auroc(scores, labels): [cite: 61]
